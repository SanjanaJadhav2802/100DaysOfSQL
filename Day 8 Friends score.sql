--Friends score
--Total friends score should be greater than 100
...
select f.pid,sum(p.Score) as friends_score,count(*) as total_friends
from friend f inner join
person p on p.PersonID = f.fid
group by f.pid
having sum(p.Score)>100
)
select s.*,p.Name
from score_details s inner join 
person p on s.pid =p.PersonID
